 ...
 */
public class DependencyResolvePlugin implements Plugin<Project> {
    @Override
    public void apply(Project targetProject) {
        ...
        targetProject.afterEvaluate {
            ...
            targetProject.configurations.all { DefaultConfiguration configuration ->
            ...
            println("dynamicDependency map : " + resolveExtMap)
            // 之前的写法是这样的，死活替换不成功，去掉了闭包里面的参数就可以了。学习一下这两种的区别。
//            targetProject.configurations.all { DefaultConfiguration configuration ->
            targetProject.configurations.all {
                resolutionStrategy.dependencySubstitution {
                    resolveExtMap.each { key, value ->
                        println("正在替换依赖， 当前工程：" + targetProject.name)
